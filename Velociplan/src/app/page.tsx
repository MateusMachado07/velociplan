...
const faqs = [
  ...
  },
  {
    ...
    resposta:
      "Normalmente entre 20 a 40 segundos. A IA analisa o teu perfil e cria um plano completo e personalizado — não é um template genérico.",
  },
  {
...
const steps = [
  ...
  { numero: "02", titulo: "A IA cria o teu plano", texto: "Em 20-40 segundos recebes um plano completo, semana a semana, feito para ti." },
    ...
    <main className="min-h-screen bg-brand-navy">
      ...
      <nav className="border-b border-white/10 px-6 py-4 sticky top-0 bg-brand-navy/95 backdrop-blur-sm z-10">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <Image src="/logo.png" alt="VelociPlan" width={140} height={42} />
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-6 mt-10 text-sm text-gray-500">
            <span className="flex items-center gap-2">
              <span className="text-green-400">✓</span> Plano em 20-40 segundos
            </span>
            <span className="flex items-center gap-2">
      ...
      <section className="px-6 py-12">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="rounded-2xl border border-white/10 overflow-hidden shadow-2xl shadow-black/40">
            {/* PDF header mockup */}
            <div className="bg-[#0D1B2A] px-6 py-5 border-b border-white/10">
              <div className="text-brand-blue font-black text-xl mb-1">VelociPlan</div>
              <div className="text-white font-bold text-base">Plano de Treino — 8 Semanas</div>
              <div className="text-gray-500 text-xs mt-1">Plano personalizado gerado por Inteligência Artificial</div>
            </div>
            {/* PDF content mockup */}
            <div className="bg-white/[0.03] px-6 py-5 space-y-4">
              {/* Section */}
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-1 h-4 bg-brand-blue rounded-full" />
                  <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Visão Geral do Plano</span>
                </div>
              ...
              <div className="rounded-lg overflow-hidden border border-white/10">
                <div className="bg-[#0D1B2A] px-4 py-2 flex justify-between items-center">
                  <span className="text-white text-xs font-bold">Semana 1</span>
                  <span className="text-brand-blue text-xs">Base Aeróbica</span>
                </div>
                <div className="divide-y divide-white/5">
                  {["Segunda-feira", "Terça-feira", "Quarta-feira"].map((dia, i) => (
                    <div key={dia} className="px-4 py-2.5 flex items-center gap-3">
                      <span className="text-[10px] font-bold text-gray-500 uppercase w-24 shrink-0">{dia}</span>
                      {i === 0 ? (
                        <div className="flex-1 space-y-1">
                          <div className="h-2 bg-brand-blue/40 rounded-full w-20" />
                          <div className="h-2 bg-white/10 rounded-full w-32" />
                        </div>
                      ) : (
                        <span className="text-xs text-gray-600 italic">Descanso</span>
                      )}
                    </div>
                  ))}
                </div>
              </div>
              {/* Nutrition section mockup */}
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-1 h-4 bg-brand-blue rounded-full" />
                  <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Nutrição e Recuperação</span>
                </div>
                <div className="space-y-1">
                  <div className="h-2.5 bg-white/10 rounded-full w-full" />
